...
2026-09-09 15:55 · memory · 3 lessons recorded (docs/knowledge/toolchain/pnpm-11-reads-pnpm-workspace-yaml-not-package-json.md, docs/knowledge/toolchain/eslint-must-ignore-claude-and-tool-dirs.md, docs/knowledge/harness/pr-sh-summary-evidence-must-be-a-string.md); index regenerated. No ADR: nothing decision-level moved (Node 22 reading is already in Progress above). Follow-ups (ADR-0001 .ts→.mjs correction, check-boundaries regex→AST, U-007 ajv/yaml vs ADR-0001 zero-deps, pr.sh jq array support) stay as Progress notes for the units that own them, not spun into new unit files here.
