...
2026-09-09 17:30 · memory · both reviewer verdicts pass; the collector's DoD-1 failure was against the pre-round-1 literal command and integration.log now shows that command green. Follow-ups for a future unit (not this PR): the schema drift test (packages/control/test) compares table names only — extend it to compare information_schema.columns against the Drizzle definitions so a renamed column is caught; tool/check-allowed.mjs should always permit pnpm-lock.yaml and pnpm-workspace.yaml, since any unit that adds a dependency touches both (pnpm-workspace.yaml joins the existing pnpm-lock.yaml note in docs/knowledge/harness/). No ADR needed: ADR-0004, ADR-0008 and ADR-0011 were implemented as written, not moved. See docs/knowledge/control/ (new area) and docs/knowledge/toolchain/ for lessons recorded from this unit. PR: https://github.com/kapdroid/Fa-Lens/pull/22
