...
2026-09-09 19:11 · build · gate --fast green at a20de4e; red.log 17 failing before, test.log 18 green after, coalesce.log 3 green against a real Postgres. Two corrections while building. First, I put the coalescing test under packages/control and added @falens/service to that package, which reverses the dependency direction architecture §2 sets — the boundary checker did not catch it because it reads src, not package.json or tests. The test now lives in packages/service/test/integration with its own container harness, and the DoD path was corrected to match. Second, call() now has typed overloads so a caller of a known verb gets that verb's output type, and bindings takes the control plane's pool type from its own signature rather than importing a driver into the service
2026-09-09 19:16 · review · adr-reviewer pass (2 should, 2 notes), all addressed. The sharpest: my DoD said the contract test proves a skin cannot invent a verb, but nothing asserted it — the package surface is now checked for verb objects and a register function, and the registry is frozen so it cannot be extended at run time either. Also fixed a real tenancy gap the reviewer found by reading rather than running: the reads validated a whole scope and used none of it, so a request naming another company would have been answered from the bound one; it is now refused before any handler runs. Filed U-028 (the boundary checker reads manifests and test imports, the blind spot that let me reverse a dependency earlier today) and U-029 (a transaction-aware create so the service stops writing SQL for a table it does not own). Still open and recorded: these reads ignore the date range, which U-015 owns
2026-09-09 19:16 · pr · https://github.com/kapdroid/Fa-Lens/pull/26
2026-09-09 19:30 · memory · knowledge captured under docs/knowledge/service/ (registry exclusivity must be checked not declared; a schema that validates a field the handler ignores is a false promise; a layering rule holds only where something checks it). No ADR — no decision moved. Follow-ups U-028 and U-029 already filed by the build step, not duplicated; the open note that reads ignore the date range is owned by U-015. PR: https://github.com/kapdroid/Fa-Lens/pull/26
