...
2026-09-09 17:05 · memory · 3 lessons recorded under docs/knowledge/kernel/ (severity order and count tally; kernel purity via executor callback and reported duration; delta-must-win-equals in assertion kind lookup); index regenerated. No ADR: ADR-0003's assertion vocabulary was implemented as written, not changed. Follow-ups for later units, not new unit files here: U-009 owns splitWindows and the 31-day cap (ADR-0014, correctly out of scope here); the `script` step kind stays refused-as-error until the sandbox unit (isolated-vm, ADR-0003) lands. status stays review; PR https://github.com/kapdroid/Fa-Lens/pull/20
