...
2026-09-09 18:40 · memory · spec-checker and adr-reviewer both pass; ADR-0016 already proposed in this branch with its README row, no new ADR written. Four lessons recorded under docs/knowledge/control/: run coalescing is a gate not a lease, lock-scoped work must share the lock's own transaction, a swappable seam must not leak its implementation, LISTEN/NOTIFY reconnect must re-issue LISTEN. Two follow-ups drafted: U-012 (reword architecture.md §4 per ADR-0016) and U-013 (test pg-boss job expiry, not just retry delay) — architecture.md itself not touched here, that's U-012's job. pr: https://github.com/kapdroid/Fa-Lens/pull/24
2026-09-09 18:27 · memory · four knowledge notes under docs/knowledge/control; no ADR beyond ADR-0016 in this branch. The scribe drafted the two follow-ups as U-012 and U-013, ids the stage-5 roadmap already gives to the adapter units, so they were renumbered U-024 (reword architecture §4 per ADR-0016) and U-025 (test pg-boss job expiry, the hang timeout, not just retry delay)
