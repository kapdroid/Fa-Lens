...
dod:
  ...
  - "`vitest packages/control/test/integration/queue.test.ts` passes: a queued job whose handler crashes is delivered again after its expiry, and a completed job is not"
