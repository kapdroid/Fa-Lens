...
2026-09-09 11:10 · memory · reviewers pass (spec-checker, adr-reviewer, 0 items each); no ADR moved (ADR-0011 implemented as-is). Knowledge notes: docs/knowledge/harness/tool-selftest-when-tests-dir-not-allowed.md, docs/knowledge/harness/evidence-for-ledger-mutating-dod.md, docs/knowledge/harness/intake-accepts-merged-review-dependency.md (covers the two PR #6 harness defects); README index updated. Follow-ups already tracked: gate wiring for selftest/render --check has no unit file yet (out of this unit's scope per plan step 4); the status-review→done-after-merge gap is the U-001 (gate-runs-tool-tests) theme, no new draft needed
