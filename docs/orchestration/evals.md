...
| 2026-09-09 | E-005 | pass | e1cf028 | first native /build run (slash command); tier 1 harness unit through the full loop: intake → explore → isolate → plan (spec-checker pass) → build (selftest 8 red → 8 green) → verify (3/3) → adr-reviewer pass, 0 items → PR (link in the unit's Progress). Harness defects found and fixed in PR #6: nothing flipped status review→done after a merge, so intake refused the merged dependency E-002 (intake now accepts a review dependency whose unit branch is merged into origin/main); .falens-unit was tracked on main despite .gitignore (untracked). |
<!-- results:end -->

Note on agent invocation: E-001 and E-002 ran before the project agents were loadable in the session that created them, so their reviewers were general-purpose subagents seeded with the agent files. From E-003 on, the agents ran natively (`subagent_type: adr-reviewer`, `design-reviewer`, `adapter-safety-reviewer`, `fresh-eyes`, `evidence-collector`, `memory-scribe`, `explorer`, `spec-checker`), which is the intended path. From E-005 on, `/build` ran natively as a slash command in a fresh session. Results are appended with `node tool/evals.mjs record <id> <pass|fail|blocked> <note>` and the table above is regenerated with `node tool/evals.mjs render` from `evals.jsonl`; the table is never edited by hand.
