...
- **Layer 0, budgets (v1):** ADR-0005 budgets per server; **replicas only** (confirmed available for every source; primaries are not in the catalog); keyset paging; column allow-lists; **interactive runs capped at 31 days**. Larger ranges are refused interactively and offered as background jobs.
