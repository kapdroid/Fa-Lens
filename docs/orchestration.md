...
Budget per unit: at most 6 agent calls (tier 1–2) or 8 (tier 3), and 2 fix rounds. Exceeding either is a stop, not a retry. Every state writes its result to the unit file or `evidence/`, so a new session resumes from the last Progress line.
